...
describe('quiz attempt runtime persistence', () => {
  beforeEach(() => {
    // Node 25 exposes navigator.locks. These race tests intentionally exercise
    // the browser fallback path where Web Locks are unavailable.
  ...
  it('lets a complete submission supersede a pending draft', async () => {
    const order: string[] = [];
    ...
    expect(order).toEqual(['start:submitted', 'end:submitted']);
  });

  it('drops queued intermediate drafts while preserving the latest snapshot', async () => {
    vi.useFakeTimers();
    let releaseFirst!: () => void;
    const writes: QuizAttemptRecordInput[] = [];
    const write = vi.fn(async (input: QuizAttemptRecordInput) => {
      writes.push(input);
      if (writes.length === 1) {
    ...
    const writer = createQuizAttemptWriter({ debounceMs: 0, write });
    const base = { stageId: 'stage-1', sceneId: 'scene-quiz', attemptId: 'attempt-1' };

    writer.scheduleDraft({ ...base, answers: { q1: 'A' } });
    await vi.advanceTimersByTimeAsync(0);
    expect(writes).toHaveLength(1);
    writer.scheduleDraft({ ...base, answers: { q1: 'A', q2: 'B' } });
    writer.scheduleDraft({ ...base, answers: { q1: 'A', q2: 'B', q3: 'C' } });
  ...
  it('keeps authoritative reads behind phases waiting in the writer tail', async () => {
    ...
    writer.scheduleDraft(base);
    const flushing = writer.flushDraft();
    await didStartDraft;
