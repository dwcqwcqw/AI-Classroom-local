...
describe('quiz view runtime hydration', () => {
  ...
  it('recognizes the anonymous persistence boundary without swallowing other failures', () => {
    expect(
      isPersistenceAuthenticationError(new Error('Authentication is required for persistence')),
    ).toBe(true);
    expect(isPersistenceAuthenticationError(new Error('storage unavailable'))).toBe(false);
    expect(isPersistenceAuthenticationError('Authentication is required for persistence')).toBe(
      false,
    );
  });
