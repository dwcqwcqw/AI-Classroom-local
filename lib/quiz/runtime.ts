 ...
 * A submitted/reviewed snapshot already contains the complete answer set, so it
 * invalidates drafts that have not started instead of waiting for every
 * intermediate answer snapshot to reach the server.
 */
export function createQuizAttemptWriter(options: QuizAttemptWriterOptions = {}): QuizAttemptWriter {
  ...
  const run = (
    ...
    const operation = tail.then(() => (shouldWrite() ? write(input) : undefined));
  ...
  const flushDraft = (): Promise<void> => {
    ...
    return run({ ...input, phase: 'draft' }, () => revision === draftRevision);
  };
