...
export function isPersistenceAuthenticationError(error: unknown): boolean {
  return (
    error instanceof Error && /authentication is required for persistence/i.test(error.message)
  );
}
