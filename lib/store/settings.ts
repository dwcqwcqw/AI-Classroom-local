            ...
            set((state) => {
              ...
              for (const [pid, info] of Object.entries(data.providers)) {
                ...
                if (newProvidersConfig[key]) {
                  ...
                  newProvidersConfig[key] = {
                    ...newProvidersConfig[key],
                    // A managed provider must never keep a browser-persisted
                    // credential or endpoint. Besides being stale, masked
                    // values such as `••••` are not valid ByteString header
                    // values and can make every provider test fail before the
