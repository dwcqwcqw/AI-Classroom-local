...
import { contentTypeForPath, persistClassroomAsset } from '@/lib/server/classroom-assets';
  ...
  const generateVideos = async () => {
    for (const req of videoRequests) {
      try {
        ...
        await persistClassroomAsset(classroomId, `media/${filename}`, buf, 'video/mp4');
        mediaMap[req.elementId] = mediaServingUrl(baseUrl, classroomId, `media/${filename}`);
