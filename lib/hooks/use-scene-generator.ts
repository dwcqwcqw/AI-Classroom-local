...
// Background course continuation values forward progress over repeatedly
// blocking the whole queue on one slow provider response. The foreground first
// page still supplies its own retry policy from generation-preview.
const BACKGROUND_SCENE_RETRY_OPTIONS = { maxRetries: 0 } as const;
...
function withThinkingConfig<T extends Record<string, unknown>>(body: T): T {
  if ('thinkingConfig' in body) return body;
      ...
      try {
        const fetchContent = (outline: SceneOutline) =>
          fetchSceneContent(
            ...
            signal,
            BACKGROUND_SCENE_RETRY_OPTIONS,
          );
        ...
        for (const outline of pending) {
          ...
          try {
            ...
            if (!contentResult.success || !contentResult.content) {
              ...
              // A failed page must never block the rest of the course. Keep it in
              // failedOutlines for an explicit retry, remove its live placeholder,
              // and continue consuming the remaining queue in both serial and
              // parallel modes.
            ...
            if (actionsResult.success && actionsResult.scene) {
              ...
              // TTS generation is best-effort; the scene itself remains usable.
              if (
                settings.ttsEnabled &&
                ...
                if (!ttsResult.success) {
                  ...
                  // Narration is an enhancement, not a page-generation gate. The
                  // scene remains usable and the public viewer can lazily synthesize
                  // its speech later from the persisted action text.
            ...
            const message =
              error instanceof Error ? error.message : 'Unexpected scene generation failure';
            log.warn('Scene generation failed; continuing with remaining pages:', {
        ...
        if (!abortRef.current && !pausedByAbort) {
          if (hadSceneFailure) {
            // The healthy pages are complete. Keep the run paused only to expose
            // the isolated failed pages for retry; no later page is withheld.
            store.getState().setGenerationStatus('paused');
            store.getState().setGeneratingOutlines([]);
          } else {
            store.getState().setGenerationStatus('completed');
