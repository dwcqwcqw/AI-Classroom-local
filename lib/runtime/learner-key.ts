...
export function getLearnerKey(kv?: KVStore): Promise<string> {
  ...
  return getDeviceLearnerKey();
}

/** Resolve the per-device anonymous key without consulting account-backed configuration. */
export function getDeviceLearnerKey(): Promise<string> {
