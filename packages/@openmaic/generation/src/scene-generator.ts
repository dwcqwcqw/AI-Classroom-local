...
function extractHtml(
  ...
  const isCompleteDocument = (candidate: string): boolean => {
    if (!/<\/html\s*>/i.test(candidate) || !/<\/body\s*>/i.test(candidate)) return false;
    const scriptStarts = candidate.match(/<script\b[^>]*>/gi)?.length ?? 0;
    const scriptEnds = candidate.match(/<\/script\s*>/gi)?.length ?? 0;
    return scriptStarts === scriptEnds;
  };
  ...
  if (start !== -1) {
    ...
    if (htmlEnd !== -1) {
      const candidate = response.substring(start, htmlEnd + 7);
      if (isCompleteDocument(candidate)) return candidate;
    }
  }
