...
export type DocumentHttpStoreResolver<
  TScene extends SceneLike = Scene,
  TStage extends Stage = Stage,
> = (
  store: DocumentStore<TScene, TStage>,
  principal: DocumentHttpPrincipal,
) => DocumentStore<TScene, TStage> | Promise<DocumentStore<TScene, TStage>>;
...
> {
  ...
  /** Resolve a request-scoped store after authentication, for tenant isolation. */
  resolveStore?: DocumentHttpStoreResolver<TScene, TStage>;
...
async function route<TScene extends SceneLike, TStage extends Stage>(
  ...
  res: ServerResponse,
  store: DocumentStore<TScene, TStage>,
  options: DocumentHttpHandlerOptions<TScene, TStage>,
): Promise<void> {
  ...
  const requestStore = options.resolveStore ? await options.resolveStore(store, principal) : store;
  ...
  if (parts.length === 1 && method === 'GET') {
    sendJson(res, 200, await requestStore.listDocuments());
    return;
  }
  ...
  if (parts.length === 2 && method === 'PUT') {
    ...
    try {
      await requestStore.saveDocument(document);
    } catch (error) {
  ...
  if (parts.length === 2 && method === 'GET') {
    const document = await requestStore.loadDocument(stageId);
  ...
  if (parts.length === 2 && method === 'DELETE') {
    await requestStore.deleteDocument(stageId);
    sendNoContent(res);
    return;
  }
  if (parts.length === 3 && parts[2] === 'stage' && method === 'PUT') {
    ...
    try {
      await requestStore.putStage(stageId, stage);
    } catch (error) {
      classifyStoreError(error);
    }
    sendNoContent(res);
  ...
  if (parts.length === 4 && parts[2] === 'scenes') {
    ...
    if (method === 'PUT') {
      ...
      try {
        await requestStore.putScene(stageId, scene);
      } catch (error) {
        classifyStoreError(error);
      }
      sendNoContent(res);
    ...
    if (method === 'GET') {
      const scene = await requestStore.getScene(stageId, sceneId);
    ...
    if (method === 'DELETE') {
      try {
        await requestStore.deleteScene(stageId, sceneId);
      } catch (error) {
        classifyStoreError(error);
      }
      sendNoContent(res);
