...
import type { DocumentHttpStoreResolver } from '../src/server/document.js';
...
function makeHarness(
  ...
  options: {
    ...
    resolveStore?: DocumentHttpStoreResolver;
  } = {},
) {
  ...
  const handler = createStorageHttpHandler(runtime, documents, {
    ...
    ...(options.resolveStore === undefined ? {} : { resolveStore: options.resolveStore }),
...
describe('HttpDocumentStore contract mapping', () => {
  ...
  test('resolves a request-scoped document store after authentication', async () => {
    let calls = 0;
    const { client } = makeHarness(() => true, {
      resolveStore: (store, principal) => {
        calls += 1;
        expect(principal.learnerKey).toBe('author');
        return store;
      },
    });
