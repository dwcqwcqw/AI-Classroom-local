...
interface PlaybackChromeRootProps {
  ...
  /** Anonymous, distraction-free course share surface. */
  readonly publicViewer?: boolean;
}
...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot(
    { onRetryOutline, canEnterProMode, onEnterProMode, publicViewer = false },
    ref,
  ) {
    const { t } = useI18n();
    const {
      ...
      const initializeScene = async () => {
        ...
        const engine = new PlaybackEngine([currentScene], actionEngine, audioPlayerRef.current, {
          ...
          onComplete: () => {
            ...
            if (publicViewer || autoPlayLecture) {
              setTimeout(() => {
                const stageState = useStageStore.getState();
                if (!publicViewer && !useSettingsStore.getState().autoPlayLecture) return;
    ...
    const sceneViewerHeight = (() => {
      const headerHeight = isPresenting || publicViewer ? 0 : 80;
      const roundtableHeight = mode === 'playback' && !isPresenting && !publicViewer ? 192 : 0;
    ...
    return (
      <div
        ...
        onTouchEnd={(event) => {
          if (!publicViewer || touchStartXRef.current === null) return;
          const endX = event.changedTouches[0]?.clientX;
          if (endX === undefined) return;
          const delta = endX - touchStartXRef.current;
          touchStartXRef.current = null;
          if (Math.abs(delta) < 48) return;
          if (delta < 0) handleNextScene();
          else handlePreviousScene();
        }}
        className={cn(
          'flex-1 flex overflow-hidden bg-gray-50 dark:bg-gray-900',
          publicViewer && 'h-full w-full touch-pan-y bg-black dark:bg-black',
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
          ...
          >
            <CanvasArea
              ...
              hideToolbar={
                publicViewer || mode === 'playback' || (isPresenting && !controlsVisible)
              }
