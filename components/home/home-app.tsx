...
import { FocusedPageTypeSelector } from '@/components/generation/focused-page-type-selector';
...
const SlideThumbnail = dynamic(
  () =>
    import('@/components/slide-renderer/SlideThumbnail').then((module) => module.SlideThumbnail),
  { ssr: false },
);
...
async function readClassroomJobResponse(response: Response) {
  const data = (await response.json()) as ClassroomGenerationJobResponse;
...
async function generateFocusedClassroom(requirement: string, pageType: FocusedPageType) {
  const created = await readClassroomJobResponse(
  ...
  if (!created.jobId) throw new Error('Generation job did not return an id');
  const pollUrl = created.pollUrl || `/api/generate-classroom/${created.jobId}`;

  // Focused pages normally finish in well under a minute. Keep polling long
  // enough for an unusually slow model response without imposing a quality-
  // reducing generation deadline.
  for (let attempt = 0; attempt < 300; attempt += 1) {
    await new Promise((resolve) => window.setTimeout(resolve, 1000));
    const job = await readClassroomJobResponse(await fetch(pollUrl, { cache: 'no-store' }));
    if (job.status === 'failed') {
      throw new Error(job.error || 'Classroom generation failed');
    }
    if (job.status === 'succeeded' && job.result?.url) return job.result;
  }

  throw new Error(
    'Classroom generation is still running. Please try opening it from recent courses.',
  );
}
...
function HomePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // The product's five focused page types do not need the legacy client-side
      // outline/action/media pipeline. Send an attachment-free request straight
      // to the durable server job, which generates one scene, persists it, and
      // returns the shareable classroom URL.
      if (frozenMaterials.length === 0) {
        const result = await generateFocusedClassroom(form.requirement.trim(), form.pageType);
      ...
      const requirements: UserRequirements = {
        ...
        interactiveMode: form.pageType !== 'quiz',
