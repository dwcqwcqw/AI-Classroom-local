...
import {
  ...
  isPersistenceAuthenticationError,
  isQuizRuntimeReady,
  persistQuizReview,
  persistQuizRetry,
...
async function gradeShortAnswerQuestion(
  ...
  try {
    ...
    if (!signal?.aborted) log.error('[quiz-view] AI grading failed for', q.id, err);
...
export function QuizView({ questions, sceneId, stageId }: QuizViewProps) {
  ...
  useEffect(() => {
    ...
    void loadQuizAttemptState({ stageId, sceneId })
      ...
      .catch(async (error) => {
        ...
        if (cancelled) return;
        if (isPersistenceAuthenticationError(error)) {
          try {
            const learnerKey = await getDeviceLearnerKey();
            if (cancelled) return;
            ...
            setRuntimeGate({
              status: 'ready',
              attemptId: quizAttemptId(stageId, sceneId, learnerKey),
            });
            return;
          } catch (fallbackError) {
            log.warn('Failed to start anonymous quiz runtime:', fallbackError);
          }
        }
  ...
  const handleSubmit = useCallback(() => {
    ...
    const submission = { stageId, sceneId, attemptId, answers };

    if (shortAnswerQs.length === 0) {
      const localResults = gradeChoiceQuestions(questions, answers);
      setResults(localResults);
      setPhase('reviewing');

      // Choice grading is entirely local. Queue durable writes after painting
      // the review so network latency never delays the learner's score.
      void Promise.all([
        persistQuizSubmission(submission, runtimeWriter),
        persistQuizReview({ ...submission, results: localResults }, runtimeWriter),
      ]).catch((error) => log.warn('Failed to persist quiz result:', error));
      return;
    }

    setPhase('grading');
    void persistQuizSubmission(submission, runtimeWriter).catch((error) =>
      log.warn('Failed to persist quiz submission:', error),
    );
  }, [attemptId, answers, questions, runtimeWriter, sceneId, stageId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setResults(ordered);
      setPhase('reviewing');
      void persistQuizReview(
        { stageId, sceneId, attemptId, answers, results: ordered },
        runtimeWriter,
      ).catch((error) => log.warn('Failed to persist quiz review:', error));
    })();
  ...
  }, [attemptId, persistenceAvailable, retrying, runtimeWriter, sceneId, stageId, viewLifetime]);
