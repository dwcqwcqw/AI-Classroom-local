...
import { buildFastOutline } from '@/lib/server/classroom-generation';
const log = createLogger('Outlines Stream');

export const maxDuration = 300;

function focusedOutlineResponse(requirements: UserRequirements): Response {
  const result = buildFastOutline(requirements.requirement, requirements.pageType);
  const outline = result.outlines[0]!;
  const events = [
    { type: 'languageDirective', data: result.languageDirective },
    { type: 'courseTitle', data: result.courseTitle },
    { type: 'outline', data: outline, index: 0 },
    {
      type: 'done',
      outlines: [outline],
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // A selected focused type is already a complete one-page outline. Avoid
    // spending a model call to rediscover the type and page count the user
    // explicitly chose.
    if (requirements.pageType) {
      return focusedOutlineResponse(requirements);
    }

    // Get API configuration only for the legacy multi-page outline path.
    const {
      model: languageModel,
      modelInfo,
      modelString,
