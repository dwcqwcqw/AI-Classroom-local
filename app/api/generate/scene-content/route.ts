...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const focusedOutputTokens = requirements?.pageType
      ? Math.min(modelInfo?.outputWindow ?? 5_500, 5_500)
    ...
    const aiCall = async (
      ...
      const effectiveUserPrompt = requirements?.pageType
        ? `${userPrompt}\n\nFocused one-page delivery: keep the implementation compact and self-contained. Include only the learning interaction, essential controls, responsive layout, and required widget contract. Avoid decorative sections, narration, media, and explanatory filler. Do not sacrifice functional correctness.`
        : userPrompt;
        ...
        const result = await callLLM(
          {
            ...
            messages: [
              {
                role: 'user' as const,
                content: buildVisionUserContent(effectiveUserPrompt, resolvedImages),
              },
            ],
            maxOutputTokens: focusedOutputTokens,
            maxRetries: 0,
          },
          'scene-content',
          undefined,
