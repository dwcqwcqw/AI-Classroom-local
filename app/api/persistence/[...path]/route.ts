...
import { isPersistenceAuthConfigured } from '@/lib/persistence/server-auth';
import {
...
async function createPersistenceHandler(
  ...
  // Supabase authentication derives document, runtime, and asset identity from
  // the verified user id. The legacy development token remains available only
  ...
  return createStorageHttpHandler(runtimeStore, documentStore, {
    ...
    resolveStore: (store, principal) => {
      if (!principal.learnerKey) throw new Error('Authenticated document principal is missing');
      return scopeDocumentStore(store, principal.learnerKey);
    },
...
export async function handlePersistenceRequest(
  ...
  if (!isPersistenceAuthConfigured()) {
    return jsonError(
      503,
      'PERSISTENCE_AUTH_MISSING',
      'server persistence authentication is not configured',
    );
  }
